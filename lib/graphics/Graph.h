...
public:
	...
	{
		if (a.attached)ThrowError("attempt to copy attached shape");
	}
	*/
...
struct Rectangle : Shape {
	...
	{
		if (h<=0 || w<=0) ThrowError("Bad rectangle: non-positive side");
		add(xy);
	}
	Rectangle(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }
	{
		if (h<=0 || w<=0) ThrowError("Bad rectangle: first point is not top left");
		add(x);
	}
...
struct Lines : Shape {	// indepentdent lines
	Lines() {}
	Lines(std::initializer_list<Point> lst) : Shape{lst} { if (lst.size() % 2) ThrowError("odd number of points for Lines"); }
	void draw_lines() const;
