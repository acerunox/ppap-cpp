...
| `character classification ` | 'membership' to a particular class of characters, e.g. alphanumeric, whitespace etc.        |
| `decimal                  ` | base-10 number system                                                                       |
| `defaultfloat             ` | flag used to set floating-point format, default format                                      |
| `file positioning         ` | changing the position from which characters will be extracted from a file                   |
| `fixed                    ` | flag used to set floating-point format, fixed format                                        |
| `hexadecimal              ` | base-16 number system                                                                       |
| `irregularity             ` | not treating everything uniformly, e.g. treating input sources differently                  |
| `line-oriented input      ` | input that reads an entire line instead of a single value of a particular type              |
| `manipulator              ` | helper function used for output formatting                                                  |
| `nonstandard separator    ` | custom separator (delimiter), not a whitespace                                              |
| `noshowbase               ` | flag that affects output of integers (doesn't show the base prefix)                         |
| `octal                    ` | base-8 number system                                                                        |
| `output formatting        ` | showing output in a particular way                                                          |
| `regularity               ` | treating everything uniformly and equivalently, imposing a single standard for things       |
